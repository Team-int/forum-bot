  ...
  run: async (client, message, args, ops) => {
    ...
    if (statFile.curr.find((x) => x.id == message.author.id)) {
      statFile.anal[message.author.id].time +=
        new Date() - statFile.curr.find((x) => x.id == message.author.id).startTime
      const embed = new Discord.MessageEmbed()
        ...
        .addField(
          '총 일한 시간',
          `${Math.floor(statFile.anal[message.author.id].time / 86400000)}일 ${Math.floor(
            parseInt(
              Math.floor(
                Math.floor(
                  statFile.anal[message.author.id].time -
                    Math.floor(statFile.anal[message.author.id].time / 86400000) * 86400000
                ) / 3600000
              )
                .toString()
                .split('.')[0]
            ...
            parseInt(
              Math.floor(
                Math.floor(
                  statFile.anal[message.author.id].time -
                    Math.floor(statFile.anal[message.author.id].time / 3600000) * 3600000
                ) / 60000
              )
                .toString()
                .split('.')[0]
            ...
            parseInt(
              Math.floor(
                Math.floor(
                  statFile.anal[message.author.id].time -
                    Math.floor(statFile.anal[message.author.id].time / 60000) * 60000
                ) / 1000
              )
                .toString()
                .split('.')[0]
      ...
      statFile.curr.splice(
        statFile.curr.indexOf(statFile.curr.find((x) => x.id == message.author.id)),
        1
      )
