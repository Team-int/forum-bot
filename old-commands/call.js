...
module.exports = {
  ...
  run: async (client, message, args, ops) => {
    if (!args[1] && args[1] != 'int' && args[1] != 'CSH' && args[1] != 'mswgen')
      return message.channel.send('호출할 대상을 입력해주세요.')
    if (ops.callTarget[message.author.id] == args[1])
      return message.channel.send('자기 자신은 호출할 수 없어요.')
    if (!message.member.roles.cache.has(ops.teamRole))
      return message.channel.send('팀원만 사용할 수 있어요.')
    if (client.callQueue.get(message.author.id))
      return message.channel.send('조금 있다가 다시 해보세요.')
    ...
    collector.on('end', async (collected) => {
      if (collected.first().emoji.name == '✅') {
        ...
        embed
          .setTitle(`${args[1]} 님을 호출하고 있어요...`)
          .setColor('RANDOM')
          .setDescription(
            `아래 반응을 누르거나 ${args[1]} 님이 메세지를 입력하거나 30분이 지나면 호출이 종료돼요.`
          )
          .setFooter(message.author.tag, message.author.displayAvatarURL())
          .setTimestamp()
        await m.edit(embed)
        ...
        for (let sub of callFile.subscriptions) {
          webpush.sendNotification(
            ...
            JSON.stringify({
              title: '호출됨',
              body: `${message.author.tag}(${message.author.id})님이 ${message.channel.name}에서 ${
                args[1]
              } 님을 호출했어요.\호출 이유: ${args.slice(2).join(' ') || '없음'}`,
        ...
        let calling = setInterval(() => {
          for (let sub of callFile.subscriptions) {
            webpush.sendNotification(
              ...
              JSON.stringify({
                title: '호출됨',
                body: `${message.author.tag}(${message.author.id})님이 ${
        ...
        embed.setTitle('관리자 호출이 취소되었어요').setColor('RANDOM').setTimestamp()
        await m.edit({
          embed: embed
        })
      }
