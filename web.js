  ...
  start: async (client, ops) => {
    ...
    io.on('connection', (socket) => {
      socket.on('notifySubscription', (data) => {
        ...
        fs.writeFile(
          '/home/azureuser/intmanager/data/notifications.json',
      ...
      socket.on('callNotifySubscription-int', (data) => {
        ...
        dbFile.subscriptions.push(data)
        fs.writeFile(
          '/home/azureuser/intmanager/data/callint.json',
          JSON.stringify(dbFile),
          () => {}
        )
      })
      socket.on('callNotifySubscription-CSH', (data) => {
        ...
        dbFile.subscriptions.push(data)
        fs.writeFile(
          '/home/azureuser/intmanager/data/callCSH.json',
          JSON.stringify(dbFile),
          () => {}
        )
      })
      socket.on('callNotifySubscription-mswgen', (data) => {
        ...
        dbFile.subscriptions.push(data)
        fs.writeFile(
          '/home/azureuser/intmanager/data/callmswgen.json',
          JSON.stringify(dbFile),
          () => {}
        )
      })
        ...
        axios
          ...
          .then((res) => {
            if (client.guilds.cache.get(ops.guildId).members.cache.has(res.data.id)) {
              ...
              fs.writeFile(
                '/home/azureuser/intmanager/data/bugs.json',
                JSON.stringify(bugFile),
                () => {}
              )
              socket.emit('newBugSubmitted')
              client.channels.cache.get(ops.confRoomChannel).send(
                `<@&${ops.adminRole}>`,
                new Discord.MessageEmbed()
                  ...
                  .addField(
                    '버그 내용',
                    data.text.length > 1024 ? `${data.text.substr(0, 1021)}...` : data.text
                  )
                  .setFooter(
                    client.users.cache.get(res.data.id).tag,
                    client.users.cache.get(res.data.id).displayAvatarURL()
                  )
                  .setTimestamp()
              )
            } else {
        ...
        axios
          ...
          .then((res) => {
            if (client.guilds.cache.get(ops.guildId).members.cache.has(res.data.id)) {
              let bugFile = require('/home/azureuser/intmanager/data/bugs.json')
              if (
                res.data.id == bugFile.bugs.find((x) => x.id == data.id).author ||
                client.guilds.cache
                  .get(ops.guildId)
                  .members.cache.get(res.data.id)
                ...
                bugFile.bugs
                  ...
                  .comments.push({
                    by: `${res.data.username
                      .replace(/</gi, '&lt;')
                      .replace(/>/gi, '&gt;')}#${res.data.discriminator
                      .replace(/</gi, '&lt;')
                      .replace(/>/gi, '&gt;')}`,
                    isAdmin: client.guilds.cache
                      .get(ops.guildId)
                      .members.cache.get(res.data.id)
                      .roles.cache.has(ops.adminRole),
                ...
                if (
                  ...
                  client.users.cache.get(bugFile.bugs.find((x) => x.id == data.id).author).send(
                    new Discord.MessageEmbed()
                      .setTitle('새 버그 메세지')
                      .setColor('RANDOM')
                      .addField('버그 제목', bugFile.bugs.find((x) => x.id == data.id).title)
