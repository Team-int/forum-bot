...
module.exports = {
    ...
    description: '유저를 뮤트해요. (int Team 멤버만 사용 가능)',
    ...
    run: async (client, message, args) => {
        ...
        if (message.guild.member(message.mentions.users.first()).roles.cache.has('761456849680597002')) return message.channel.send('이 유저는 이미 뮤트되어있어요.');
        ...
        collector.on('end', async collected => {
            if (collected.first().emoji.name == '✅') {
                ...
                await message.guild.member(message.mentions.users.first()).roles.add('761456849680597002')
