...
module.exports = {
  ...
  description: '유저를 뮤트 해제해요. (봇 관리자만 사용 가능)',
  ...
  run: async (client, message, args, ops) => {
    ...
    collector.on('end', async (collected) => {
      if (collected.first().emoji.name == '✅') {
        ...
        await message.guild.member(message.mentions.users.first()).roles.remove(ops.mutedRole)
        await message.guild.member(message.mentions.users.first()).roles.add(ops.userRole)
        embed
          .setTitle('멤버가 뮤트 해제되었어요')
          .setColor('RANDOM')
          .spliceFields(0, 1, {
            name: '뮤트한 멤버',
        ...
        sub.forEach((s) => {
          webpush.sendNotification(
            ...
            JSON.stringify({
              ...
              })님이 ${message.guild.name}에서 뮤트 해제되었어요.`,
              icon: '/static/image/inticon-512.png'
            })
          )
        })
      } else {
        embed.setTitle('멤버 뮤트 해제가 취소되었어요').setColor('RANDOM').setTimestamp()
        await m.edit({
          embed: embed
        })
      }
