...
module.exports = {
    ...
    run: async (client, message, args, ops) => {
        ...
        if (!message.guild.member(message.mentions.users.first()).roles.cache.has(ops.mutedRole)) return message.channel.send('이 유저는 뮤트되어있지 않아요.');
            ...
            if (collected.first().emoji.name == '✅') {
                ...
                sub.forEach(s => {
                    webpush.sendNotification(s, JSON.stringify({
                        title: '멤버 뮤트 해제됨',
                        body: `${message.mentions.users.first().tag}(${message.mentions.users.first().id})님이 ${message.guild.name}에서 뮤트 해제되었어요.`,
