                        ...
                        if (!guildRes.data.find(x => x.id == ops.guildId)) {
                            ...
                            if (userRes.data.bot == true) {
                                ...
                                for (let report of bugReports.bugs) {
                                    if (report.author == userRes.data.id || client.guilds.cache.get(ops.guildId).members.cache.get(userRes.data.id).roles.cache.has(ops.adminRole)) {
                                ...
                                fs.readFile('./assets/html/bugreport.html', 'utf8', (err, data) => {
                                    ...
                                    res.end(data.replace(/{token}/gi, `${tokenRes.data.token_type} ${tokenRes.data.access_token}`).replace(/{nick}/gi, client.users.cache.get(userRes.data.id).tag).replace(/{reports}/gi, reps));
                                })
                            }
                        }
                    })
